...
def test_scan_port(mocker):
    ...
    mock_socket = mocker.patch('socket.socket')  # Mocker.patch change socket.socket object with a mock
    mock_socket.return_value.__enter__.return_value.connect_ex.return_value = 0  # Connect_ex method of the mock
    ...
    mock_socket.return_value.__enter__.return_value.connect_ex.return_value = 1  # connect_ex method to return 1,
    # indicating that the port is closed.

    with patch('builtins.print') as mock_print:
        scan_port('127.0.0.1', 81)
        mock_print.assert_any_call("Checking Port:81 on 127.0.0.1\n")
        assert not any("Port 81 is open on 127.0.0.1" in call[0][0] for call in mock_print.call_args_list)

    # Test for an exception , connect_ex method raise an exception.
    mock_socket.return_value.__enter__.return_value.connect_ex.side_effect = Exception("Test exception")

    with patch('logging.error') as mock_log:
        scan_port('127.0.0.1', 82)
        mock_log.assert_called_with("Error scanning port 82 on 127.0.0.1: Test exception")


# Run multiple times, once for each function
@pytest.mark.parametrize("func", [scan_ports_concurrently, pool_scan_ports_concurrently])
def test_concurrent_port_scans(func, mocker):
    ...
    open_ports = [call[0][1] for call in mock_scan_port.call_args_list if call[0][1] in [80, 443]]
